...
if len(sys.argv) == 4:
    ...
    print(
        "USAGE: python checkthedata.py <id,mr list> <exported fake-BIDS directory> <new BIDS valid directory"
    )
    print(sys.argv)
    exit(2)
...
def editfiledir(bidsdir, newdir, subid, ses):
    ...
    pldpcasl = {
        ...
        "RepetitionTimePreparation": 4.886,
    }
    for i in perfjson:
        dataj = readjson(i)
        ...
        writejson(
            dataj, perfdir + "/sub-" + subid + "_ses-" + ses + "_task-rest_asl.json"
        )
