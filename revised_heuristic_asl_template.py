...
pcasl_3d_mz = create_key(
    "sub-{subject}/{session}/asl/sub-{subject}_{session}_acq-3D_m0scan"
)
pcasl_3d_mp = create_key(
    "sub-{subject}/{session}/asl/sub-{subject}_{session}_acq-3D_cbf"
)
...
casl_mz = create_key(
    "sub-{subject}/{session}/asl/sub-{subject}_{session}_acq-casl_m0scan"
)
fairest = create_key(
    "sub-{subject}/{session}/asl/sub-{subject}_{session}_acq-fairest_asl"
)
fairest_mz = create_key(
    "sub-{subject}/{session}/asl/sub-{subject}_{session}_acq-fairest_m0scan"
)
...
def infotodict(seqinfo):
    ...
    def get_date(series_info):
        try:
            sortval = datetime.datetime.strptime(
                series_info.date, "%Y-%m-%dT%H:%M:%S.%f"
            )
        except:
...
MetadataExtras = {
    fm_phasediff1: {"EchoTime1": 0.00255, "EchoTime2": 0.00501},
    fm_phasediff2: {"EchoTime1": 0.00406, "EchoTime2": 0.00652},
    fm_phasediff3: {"EchoTime1": 0.00411, "EchoTime2": 0.00657},
    fm_phasediff4: {"EchoTime1": 0.00492, "EchoTime2": 0.00738},
    fm_phasediff5: {"EchoTime1": 0.00412, "EchoTime2": 0.00658},
    fm_phasediff6: {"EchoTime1": 0.00519, "EchoTime2": 0.00765},
}
